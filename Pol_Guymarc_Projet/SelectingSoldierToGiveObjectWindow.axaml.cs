...
{
    // fenêtre permettant de sélectionner un soldat pour lui donner un objet
    public partial class SelectingSoldierToGiveObjectWindow : Window
    {
        ...
        protected override void OnOpened(EventArgs e)
        {
            ...
            // récupérer les soldats pouvant recevoir cet objet
            soldierList = _guilde.WhoCanReceiveObject(_selectedobject).ToList();

            if (soldierList.Count == 0)
            {
                _selectedSoldier = new Soldier(); // soldat "vide" si aucun n'est disponible
            }
            else
            {
        ...
        private void ShowSoldierInfos(Soldier soldier)
        {
            ...
            {
                // aucun soldat disponible
                GivingObjectToSoldier.IsVisible = false;
                GoRight.IsVisible = false;
                GoLeft.IsVisible = false;
                ErrorMessage.Text = "Vous n'avez pas de soldat à qui donner cet équipement";
                SeeSoldier.IsVisible = false;
                return;
            }

            // afficher les contrôles si des soldats sont disponibles
            SeeSoldier.IsVisible = true;
            ErrorMessage.Text = string.Empty;
            GoRight.IsVisible = true;
            GoLeft.IsVisible = true;

            // certaines gemmes et objets rares ne peuvent pas être donnés
            ...
            // modification de stats en fonction de l'objet sélectionné
            if (_selectedobject is Potion potion)
            {
                SoldierPv.Text += " --> " + Math.Min(_selectedSoldier.GetActualPV() + potion.getPvHealed(), _selectedSoldier.GetMaxPv()) + " / " + _selectedSoldier.GetMaxPv() + " PV";
            }
        ...
        private async void GiveObject(object? sender, RoutedEventArgs e)
        {
            ...
            // affichage des changements de stats selon le type de soldat et objet
            if (_selectedobject is MilitaryEquipement)
            {
                if (_selectedSoldier is Bandit)
                    ObjectGivenToSoldier.Text += "\nSes points de discrétion passent de " +
                ...
                {
                    ObjectGivenToSoldier.Text += "\nSes dégâts passent de " +
                                                 _selectedSoldier.GetDamages() + " à " + (int)(_selectedSoldier.GetDamages() * 1.7);
                    ObjectGivenToSoldier.Text += "\nSes points de discrétion passent de " +
                                                 _selectedSoldier.GetDiscretionPoints() + " à " +
                ...
                {
                    ObjectGivenToSoldier.Text += "\nSes dégâts passent de " +
                                                 _selectedSoldier.GetDamages() + " à " + (int)(_selectedSoldier.GetDamages() * 1.5);
                    ObjectGivenToSoldier.Text += "\nSes points de discrétion passent de " +
                                                 _selectedSoldier.GetDiscretionPoints() + " à " +
            ...
            // donner réellement l'objet au soldat
